...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match config.command.take() {
        Some(Command::Bench { bots, games, seed, simulations, determinizations, shared_sample, mcts_depth, stage_gain, corp_stage_gain, pairings, threads, report, ratings, label }) => {
            let args =
                bench::BenchArgs { bots, games, seed, simulations, determinizations, shared_sample, mcts_depth, stage_gain, corp_stage_gain, pairings, threads, report, ratings, label };
        ...
        Some(Command::Diag {
            action: DiagAction::Tempo { games, seed, corp, runner, simulations, determinizations, turns, stage_gain, corp_stage_gain, threads, report },
        }) => {
            let args =
                diag::tempo::TempoArgs { corp, runner, games, seed, simulations, determinizations, turns, stage_gain, corp_stage_gain, threads, report };
