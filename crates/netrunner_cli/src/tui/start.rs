...
//! rule, one record rule and one deck resolver, and the screen is a way
...
/// record file they name, the player they name, and the decks they
/// default to.
...
/// one, Enter starts from any pane, Esc or `q` goes back to the menu.
...
pub fn draw(frame: &mut Frame, area: Rect, menu: &StartMenu) {
    ...
    frame.render_widget(
        Paragraph::new(vec![
            ...
            Line::from("A casual game: a move can always be taken back, and the result only moves the suggested rung"),
        ]),
        header,
    );
