...
struct Cli {
    ...
    /// How much root Dirichlet noise the self-play search mixes into its
    /// root priors: `0.0` (the default) is the searcher every measurement
    /// in this repo is taken with, AlphaZero's value is `0.25`.
    ///
    /// **Applies to self-play generation only, never to `--arena-*`.** An
    /// arena is a verdict on a network and a noisy searcher would be
    /// scoring the dice; a corpus is a sample of positions and is exactly
    /// what noise is for. See `netrunner_bots::PuctConfig::
    /// dirichlet_epsilon`.
    #[arg(long = "dirichlet-epsilon", default_value_t = 0.0)]
    dirichlet_epsilon: f32,
    /// The symmetric Dirichlet concentration `--dirichlet-epsilon` draws
    /// from. See `netrunner_bots::DEFAULT_DIRICHLET_ALPHA` for why the
    /// default is fixed rather than scaled by the branching factor.
    #[arg(long = "dirichlet-alpha", default_value_t = DEFAULT_DIRICHLET_ALPHA)]
    dirichlet_alpha: f32,
    /// Above zero, replaces `--dirichlet-alpha` with `scale / candidates`
    /// at each root — AlphaZero's `10/n`. The two are different
    /// experiments, not a sweep of one: this game's median root holds two
    /// candidates, where a fixed 0.3 is a coin flip and 10/n is nearly
    /// uniform. See `netrunner_bots::PuctConfig::dirichlet_alpha_scale`.
    #[arg(long = "dirichlet-alpha-scale", default_value_t = 0.0)]
    dirichlet_alpha_scale: f32,
...
/// The search a *corpus* is generated by: the only configuration in this
/// binary that carries root noise.
fn selfplay_config(cli: &Cli) -> PuctConfig {
    PuctConfig {
        iterations: cli.simulations.max(1),
        dirichlet_epsilon: cli.dirichlet_epsilon,
        dirichlet_alpha: cli.dirichlet_alpha,
        dirichlet_alpha_scale: cli.dirichlet_alpha_scale,
        ..PuctConfig::default()
    }
}

/// The search an arena *verdict* is taken with, which is the same one
/// every other measurement in the repo uses — no root noise, whatever
/// `--dirichlet-epsilon` says. A candidate is being scored here, and a
/// searcher that explores at random would put the dice in the verdict; the
/// two configurations are separate functions so that stays a decision
/// rather than a forgotten field. See `netrunner_bots::PuctConfig::
/// dirichlet_epsilon`.
fn arena_config(cli: &Cli) -> PuctConfig {
    PuctConfig { iterations: cli.simulations.max(1), ..PuctConfig::default() }
}
...
fn play_one_game(game_index: usize, cli: &Cli) -> Result<GameTrajectory, SelfPlayError> {
    ...
    let config = selfplay_config(cli);
...
mod tests {
    ...
    /// Root noise belongs to the generator and to nothing else. An arena
    /// leg is a verdict, and one taken with a noisy searcher would be
    /// scoring the draw rather than the network.
    #[test]
    fn root_noise_reaches_self_play_and_never_an_arena_verdict() {
        let quiet = Cli::parse_from(["netrunner_selfplay", "-n", "2", "-s", "2", "-o", "out"]);
        assert_eq!(quiet.dirichlet_epsilon, 0.0, "a run that does not ask for noise does not get any");
        assert_eq!(selfplay_config(&quiet).dirichlet_epsilon, 0.0);

        let noisy = Cli::parse_from([
            "netrunner_selfplay", "-n", "2", "-s", "2", "-o", "out", "--dirichlet-epsilon", "0.25",
        ]);
        assert_eq!(selfplay_config(&noisy).dirichlet_epsilon, 0.25);
        assert_eq!(arena_config(&noisy).dirichlet_epsilon, 0.0);
    }
