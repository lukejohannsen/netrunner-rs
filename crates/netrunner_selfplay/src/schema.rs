...
pub struct GameTrajectory {
    ...
    /// Which halves of the network the search that produced this game
    /// actually seated: `"both"`, `"priors-only"`, `"value-only"` — or
    /// empty for a run with no network at all.
    ///
    /// Recorded for the same reason `pool_fingerprint` is. A priors-only
    /// corpus and a whole-network corpus are different distributions
    /// generated by the same binary at the same widths, and nothing else
    /// on this struct distinguishes them; without the field, mixing them
    /// in one replay window would be invisible exactly the way three
    /// *Elevation* deck pools once were.
    ///
    /// `#[serde(default)]` so the four archived runs load. They read empty,
    /// which is honest — they predate the flag, and every one of them was
    /// `both`.
    #[serde(default)]
    pub model_uses: String,
}
