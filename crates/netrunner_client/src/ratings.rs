...
//! it now takes what it reads off it — a path, a name, an `--unrated`
//! decision the caller has already made — so the desktop client, which
//! has no command line, rates a game through the same code and into the
//! same file.
...
/// The bot kinds a seat can be rated under, by name. A plain enum rather
...
/// The player name a local game is rated under: the one given (a flag or
...
/// Everything `SeatRating::open` needs to know about the game about to be
/// played. A struct rather than eight arguments so the two clients that
/// build one cannot transpose the deck ids.
#[derive(Debug, Clone)]
pub struct SeatRatingSpec {
    /// The rating file, from `resolve_ratings_file`.
    pub path: PathBuf,
    /// The name the game is rated under, from `player_name`.
    pub player: String,
...
impl SeatRating {
    ...
    /// rather than after it. An unrated game never builds one.
    pub fn open(spec: SeatRatingSpec) -> Result<Self, String> {
        let ratings = LocalRatings::load(&spec.path)?;
        Ok(Self {
            path: spec.path,
            ratings,
            player: spec.player,
...
/// The player's standing on both chairs, their record against every
/// opponent, and the rung to try next, as lines — `netrunner_cli ratings`
/// prints them and both clients' Ratings screens draw them, so the three
/// cannot drift.
