...
//! acts, a secondary click opens its actions as a menu at the pointer,
...
/// A secondary click on a card opens its actions as a menu at the
/// pointer — the sheet's list, without the sheet — and the menu's button
...
#[test]
fn a_secondary_click_opens_the_actions_menu_at_the_pointer_and_its_button_submits() {
    let (mut app, _dir) = headless_client();
    start_a_game(&mut app);
    to_the_runners_turn(&mut app);
    let (card, entries) = {
