...
| `backdrops/menu.jpg` | Circuitry on gunmetal, after Fantasy Flight Games' card back: the shared backdrop, behind every menu screen without a picture of its own, painted by `scripts/paint_circuit_backdrop.py` | netrunner-rs contributors | https://github.com/lukejohannsen/netrunner-rs | GPL-3.0-or-later | the repository's `LICENSE` | project | none |
