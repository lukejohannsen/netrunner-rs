...
**Decision and next step:** loop hygiene, then the first volume run on this layout — `--masked-policy` by default, the arena at 384 games, per-chair scores in `promotions.log` (`iterations.log` already carries `as_corp`/`as_runner`). `data/selfplay` and `data/checkpoints` are empty for it. The question that run answers is whether the value head, now shown the board, beats predicting zero on the Corp chair; if it still does not, the next suspect is the target (about 40% of games end by flatline or deck-out, which no board feature sees) rather than the input.
