...
## 10. The strategy guide — DONE (25 September 2026, `feat/strategy-guide`)
Asked for by the person, from a Gemini-written outline of the game's stages, the Corp's and Runner's strategies, the factions and the Corp archetypes: a guide for a new player, beside the lessons. The lessons teach the rules; the guide is what to do with them. **`docs/strategy-guide.md` is the one text**, embedded by `netrunner_client::guide` (a reader of the six Markdown constructs it uses, into chapters of blocks of spans) and shown by both clients: the desktop's Strategy Guide screen, reached from Learn to Play (chapters as pills, the chapter scrolling beside them, each chapter's cards as buttons that read the card), and a Strategy guide row on the terminal's Learn to Play screen (chapters left, page right).

**The outline was rewritten, not transcribed.** Kept: its shape (stages read off the board rather than the turn count; scoring windows; ICE placement; early pressure; trashing the Corp's economy; the three Runner factions; Glacier, fast advance, kill and their hybrids). Corrected: Threat is an ability flag keyed to the higher score (CR 1.17.1a, 9.3.6f), not a stage; "ETR outside, sentries inside" is presented as a habit players disagree on, because no source agreed on it; "run HQ when the Corp holds cards" became agenda density, with the best HQ run the turn before a scoring window; flatline is more damage than cards in grip, and an empty stack is no loss (CR 1.7.2). Replaced: every example the outline gave that is not in this pool (Biotic Labor, Project Junebug, BOOM!, Tollbooth, Anansi) with the pool's own — fast advance here is Nanomanagement's clicks and Seamless Launch's "install one turn, score the next", and a 2-advancement agenda scores from HQ in three clicks. Added, from community writing read for it and credited in the guide's Further reading: the value of a click, tempo, economy as the contest, agenda density, last-click runs against Jinteki and NBN, mulligans, clearing tags, core damage, and the four Corp factions, which the outline did not cover.

**A card named in the guide is a link to its NetrunnerDB page, and a test holds each to a playable card in the pool under its own title** (`every_card_the_guide_names_is_in_the_playable_pool_under_its_own_title`). It found two on its first run: Hedge Fund and Sure Gamble are registered by their System Gateway codes, not the Core Set ones the guide first used. A second test refuses any construct the reader does not know, and caught two italics the first screenshot showed as asterisks; the `CR` citations are held by `rules_citations`, which already reads `docs/`. Scope stays with the pool: when Phase 1 §9 widens it, the guide can cite the cards the outline wanted.
