...
* **Static vs. Runtime State Separation:**
  ...
  * `InstalledCard` / `InstalledRunnerCard`: lightweight per-instance runtime structs held inside `GameState` (`CorpState::installed`, `RunnerState::rig`). Counters, advancement tokens and rez status belong here; the only strength kept on one is a rig card's printed `base_strength` — a current strength is asked (`continuous::breaker_strength`, `continuous::ice_strength`), and a boost with a duration is a `GameState::lingering` entry.
...
Damage about to be suffered is always *announced* (`GameEvent::AboutToResolve`, a moment a card can hear and the turn counts); a `Prevention` window opens **only** when somebody could use an interrupt on the thing about to happen — an ability whose effect is an `Effect::Prevent` that matches it, requirement met and cost affordable (`prevention::could_prevent`) — zero overhead otherwise. It is the one window that **nests**: `paid_ability_window` is a single slot, so the window a prevention opens over, and any window the game's flow opens while the players are being asked, waits in `PendingPrevention::interrupted` and comes back when the asking is over. While something is parked for prevention, `apply_action` admits an interrupt and a pass and nothing else (row 4 below, narrowed).
...
The candidate set for every event is **re-derived fresh from `GameState`** on each call — `CorpState::installed` and `RunnerState::rig` are already the single source of truth for what is in play, so there is deliberately no separate "active behaviors" registry to keep in sync. `win::check_win_conditions` follows the same pure-re-derivation convention. So does `rules::continuous`: a card's standing effects (`CardDefinition::continuous`) are scanned off the active cards (`rules::active`) at each question, never registered when a card becomes active and unregistered when it leaves. The one list that *is* stored is `GameState::lingering` — effects with a duration, which outlive the resolution that made them — and even there only the entries are stored: whether one still holds is re-derived from the state at every read (`rules::lingering`), so sweeping the list is garbage collection and no handler has to remember to end anything. A prohibition with a duration ("cannot score agendas for the remainder of the turn") and a cost change with one (Tread Lightly's +3 to rez) are entries on the same list, about a player or each piece of ice rather than one install; `continuous::cannot` is the one predicate the guard in `apply_action` and the action list both ask, and because the list is public and carried by `ClientView`, a bot's determinized sample is bound by the same prohibitions as the game it samples.
