...
3. **Watch the ratio, not the count.** Re-measured September 2026 at *Elevation* Stage 8, over the 166 card files in `data/{corp,runner}`: **30 of 71 `Effect` variants are used by exactly one card, and 5 by none**. The variant count is flat against Stage 7 and the single-use count fell by one, over nine more cards — Stage 8 added no `Effect` variant at all, growing the vocabulary of *when* something happens instead (four `Trigger`s, two `EffectRequirement`s, three `GameEvent`s). That is the shape to aim for. Stages 1–8 added twenty-three variants for seventy cards, generalised one and replaced two in place: `RezInstalled` took over `RezInstalledIgnoringCost` when Mycoweb's discounted rez forced the free and paid rez paths to converge on `engine::rez_install`, and `PlayOperation { from }` took over `PlayOperationFromHq` when Plutus replayed a transaction out of Archives. The Runner half of the set front-loads its one-off mechanics; see ROADMAP Phase 1 §8 for which single-use variants have a second card coming. Before that, after the SG card-fidelity audit over 94 files: 21 of 48 single-use, 5 unused (`RemoveTags`, `RemoveBadPublicity`, `Trace`, `PreventDamage`, `PreventTrash` — the untouched prevention/tag block; the fidelity audit deleted two variants, `PermitJackOut` and `RandomFromHq`, and added two, `PromptInstallCorpCard` and `InstallRunnerCardFromGrip`, each with its reason on the variant). The widely-reused core is healthy (`Sequence` 64 cards, `PromptChooseCards` 49, `GainCredits` 35, `PresentChoice` 34, `EffectIf` 24, `DrawCards` 23, `AddCounters` 22, `EndTheRun` 21). If the next set adds single-use variants at a rate approaching its own card count, the DSL has started tracking cards rather than mechanics — stop and build a composition primitive instead of continuing.
...
`--all-matchups` plays `matchups[index % len]`, so **`--games` below `decks::matchups().len()` leaves the tail of the pool unplayed** — at 11 Corp × 12 Runner, 96 games stop eight Corp decks in and a whole deck's cards read as zero. Size the run to at least one pass of the cross product (132 today) whenever the point of the measurement is per-card coverage.
