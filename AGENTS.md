...
**A cost is not prevented** — `Cost::TakeTags`, a card trashed to pay for an ability, the Runner's paid trash of an accessed card, the memory-limit trash, a player choosing among their *own* installs — and **a parked trash names an install handle and no card**, so it rides in a view and an event unmasked. **Whatever is parked carries "the subroutines are not finished"** through one function, `pending_choice::mark_parked_resume_subroutines`; it used to be written out at five sites for two of the four parked states, and a tag parked out of a subroutine's *choice* ended the encounter with its later subroutines unfired.
