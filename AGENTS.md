...
3. **Watch the ratio, not the count.** Measured September 2026 over the 101 card files: **20 of 47 `Effect` variants are used by exactly one card, and 5 by none** (`RemoveTags`, `RemoveBadPublicity`, `Trace`, `PreventDamage`, `PreventTrash`; a sixth, `BreakSubroutine`, was deleted by the Rules Audit; the Rules Audit's adjacent-hazards pass added one single-use variant, `GainCreditsAmount`, with its reason on the variant). The widely-reused core is healthy (`Sequence` 26 cards, `GainCredits` 24, `PromptChooseCards` 16, `PresentChoice` 14, `EndTheRun` 13, `EffectIf` 13). The earlier figure here, "14 of 66", never matched the enum — it was written and not re-measured. If the next set adds single-use variants at a rate approaching its own card count, the DSL has started tracking cards rather than mechanics — stop and build a composition primitive instead of continuing.
