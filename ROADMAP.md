...
- [x] **`PuctAgent::search` could return no actions at all, and self-play died on it — DONE.** `run_iteration_loop.py` panicked on iteration 1 at `PuctAgent::search always returns at least one action`. `search` states its contract over the caller's `view.legal_actions` (real state, side-filtered) but expanded its root from `get_action_mask` over the **determinized sample**, then reported the *intersection* of the two. Nothing made that intersection non-empty, and its `assert!` only checked the input. Now the root is seeded from `view.legal_actions` — `MctsAgent::new_root` has always done this, which is exactly why MCTS never had the bug — and `search` returns one `ActionStat` per legal action, zero-visit if the search never descended it.
  - **The observed trigger is *Tāo Salonga*, and it is unrelated to the click-action guard above.** Every empty intersection found by instrumenting a 400-game sweep (10-11 per ~52,000 root determinizations) is the same state: Tāo's `PendingDecision::ChooseCards { source: OpponentInstalled, filter: Ice(Barrier), min: 2, max: 2 }` where at least one target ICE is **unrezzed**. The Runner's view masks an unrezzed Corp ICE (`identity_visible = owner_view || rezzed`), `determinize_installed` resamples it to a random ICE, and the caller's `ToggleCardSelection { ballista }` then encodes against nothing while `ConfirmCardSelection` is rejected because `selected` names cards the sample lacks. Disjoint sets, empty result, panic.
    - The first-guess explanation — that suspending basic click actions mid-run removed an accidental safety net — was **checked and is wrong**: these states carry a parked `PendingDecision`, which `ActionBlockedByPendingDecision` has always rejected basic clicks in; two of the three states dumped had no active run at all (one on the *Corp's* turn); and the binary that panicked predated the guard commit. The guard does thin out mid-run legal sets and so raises the general risk of a narrow intersection, but it is not this trigger.
  ...
  - **Left open deliberately, and worth doing next:** the search now *tolerates* the Tāo divergence rather than modelling it — a `ToggleCardSelection` naming a card the sample lacks becomes a dead branch valued `0.0`, so the swap is never meaningfully searched. Extending `reseat_selectable_cards` from its two hard-coded zone/side pairs to "re-seat every `ToggleCardSelection` target the view already reveals" would close `OpponentInstalled`, `(Runner, OwnRAndD)`, `(Corp, OwnStack)` and facedown `OwnArchives` at once. Measured on the same 60 games, the field-preservation fixes alone moved empty intersections 11 → 10 and dropped `AcceptPendingPaidChoice` misses 4 → 0, but left 442 `ToggleCardSelection`, 196 `SelectCardToAccess` and 136 `PlayOperation` drops — none of which panic any more, all of which are search quality.
- [ ] **`legal_actions_for(Runner)` leaks an unrezzed Corp ICE's identity.** `pending_choice::zone_card_ids(OpponentInstalled)` reads `corp.installed` off the real state, so a parked `ChooseCards` over opponent installs offers the Runner `ToggleCardSelection { card_id }` naming cards their own `ClientView` masks to `card: None`. Found while diagnosing the PUCT crash above — it is *why* the sampled and real action sets diverge there. Fog of war is meant to be structural at the `ClientView` boundary, and this routes around it.
- [ ] **`ActionSpace` aliases duplicate card ids.** `bounded_position_installed` takes the *first* match, so with three copies of *Tithe* installed, `ScoreAgenda`/`AdvanceCard`/`RezIce` on the second or third copy all encode to the first copy's slot. Independent of determinization; likely behind the 96 `ScoreAgenda` encode-misses counted above. Needs a decision on whether the index space distinguishes copies at all.
