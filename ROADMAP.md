...
> **Noted, not fixed — stale off-turn clicks.** `end_turn` deliberately leaves the ending side's clicks untouched, relying on phase gating to make them inert. Paid-ability windows bypass phase gating, so a side that ends its turn with clicks left could spend them on a `Cost::Clicks` paid ability during the opponent's turn. Pre-existing (run and turn-boundary windows already allowed it) and made slightly more reachable by the post-action window, but not introduced by it. Zeroing clicks at `end_turn` is the obvious fix if it ever matters.
