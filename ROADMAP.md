...
- [x] **Simultaneous trigger order — cross-side consistency DONE (defensive).** `both_sides_candidates` now tags each candidate with its side and routes through `order_active_first` via a new `dispatcher::turn_active_side` (reads `GamePhase`, which is total — unlike `current_actor`, which is `None` during `StartOfTurn`). Previously it emitted Corp-before-Runner regardless of whose turn it was, at the only two dispatch sites whose audience spans both sides.
  ...
  - **Still open — player-chosen ordering among your *own* simultaneous triggers.** Reachable, unlike the two above: 7 cards carry `OnTurnStart`, so two installed at once makes the order observable. Needs a new `PendingDecision` variant, new `PlayerAction`(s), `ActionSpace` slots, and dispatcher threading, with the deadlock exposure that entails (see the `current_actor`-vs-`apply_action` precedence invariant, which has already caused this bug class twice). Worth doing when a card makes the order actually matter.
- [ ] **Post-action paid-ability window.** Windows exist at every run checkpoint and at turn boundaries; a generic window after an ordinary click action does not, so the **non-active** player gets no chance to use a paid ability between the active player's click actions. Rules-correct to add, but weigh the cost: it roughly doubles decision points (a step-budget risk this project has hit before), and System Gateway gives the Corp only **2** paid-ability cards (*Spin Doctor*, *Regolith Mining License*) to respond with. If built, gate it on some side actually having an activatable ability, the way `WindowCheckpoint::Prevention` already gates itself.
