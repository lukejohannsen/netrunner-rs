...
- [ ] **Simultaneous trigger order** — narrower than previously recorded here. `dispatcher::order_active_first` *already* implements the cross-side rule (active player's reactions resolve first, stable within a side), so that part is correct. Two real gaps remain:
  - **Consistency:** `order_active_first` is applied at only one of the multi-card dispatch sites. `both_sides_candidates` (used by the `OnDamageAboutToResolve`/`OnTrashAboutToResolve` prevention dispatches) hardcodes Corp-before-Runner regardless of who is active. Small and mechanical — thread the active side through, exactly as the other site does.
  - **Player-chosen ordering among your *own* simultaneous triggers** is missing entirely. This needs a new interactive decision, with the deadlock risk that entails (see the `current_actor` precedence invariant), for a case that may not even be reachable in the current card pool. Not worth doing until a card makes it matter.
