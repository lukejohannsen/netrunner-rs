...
2. **Close the PUCT Runner's remaining gap** (Phase 3 §1, last bullet): 0.516 against the heuristic Runner's 0.583 on the same Corp. The door and the mulligan are closed; what is left is the profile — the heuristic runs 30 times a game and clicks for credits 3, the PUCT Runner 26 and 9 — and the one blocker every honest fix hit: the determinization pool is the registry, 25% agendas, so a breach valued in expectation sends the Runner to the centrals. A **deck-aware determinization** (the opponent's identity is public, the published decklists are embedded) is the lead; the breach chance node (`PuctConfig::breach_outcomes`) is built and waiting on it.
