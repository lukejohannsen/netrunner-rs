...
def chairs(reports, against):
    ...
    for path, report in reports:
        for name in report["bots"]:
            if name == against:
                continue
            corp = share([g for g in report["games"] if g["corp"] == name and g["runner"] == against])
            runner = share([g for g in report["games"] if g["corp"] == against and g["runner"] == name])
            if not corp[2] or not runner[2]:
                continue
            print(f"{path.split('/')[-1]:<22} {name:<12} {corp[0]:>10.3f} ±{corp[1]:.3f}"
                  f"{1 - runner[0]:>13.3f} ±{runner[1]:.3f}")
        control = share([g for g in report["games"] if g["corp"] == against and g["runner"] == against])
        if control[2]:
            print(f"{path.split('/')[-1]:<22} {'(control) ' + against:<12} {control[0]:>10.3f} ±{control[1]:.3f}")


def main():
    parser = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("mode", choices=["curve", "chairs"])
